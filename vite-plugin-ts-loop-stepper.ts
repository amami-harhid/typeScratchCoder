...
// 外だしした関数群をインポート
import { collectTargets, createVisitor } from './vite/traverser';

export function vitePluginTsLoopStepper(): Plugin {
    return {
        ...
        transform(code, id) {
            ...
            const compilerOptions: ts.CompilerOptions = { target: ts.ScriptTarget.Latest, allowJs: true };
            ...
            // 1. 対象となる関数ノードのストック
            const targetFunctionNodes = new Set<ts.Node>();
